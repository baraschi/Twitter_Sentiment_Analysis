...
def classify_bow(train, test, tweets_col = CLEAN_TWEET, filename = "bow"):
    ...
    test_bow = bow_vectorizer.fit_transform(test[tweets_col])
...
def classify_tfidf(train, test, tweets_col = CLEAN_TWEET, filename = "tfidf"):
    tfidf_vectorizer = TfidfVectorizer(max_df=0.90, min_df=2, max_features=1000, stop_words='english')
    # TF-IDF feature matrix
    train_tfidf = tfidf_vectorizer.fit_transform(train[tweets_col])
    test_tfidf = tfidf_vectorizer.fit_transform(test[tweets_col])
    ...
    # regression using test set
    lreg_test = LogisticRegression()
    lreg_test.fit(train_tfidf, train['label']) # training the model
    prediction_test = lreg_test.predict(test_tfidf)
